...
/** 单分钟 token 用量与估算花费(小时下钻分钟分布图一根柱) */
export interface TokenUsageMinute {
  /** 本地分钟 0-59 */
  minute: number
  /** 总输入 token(含缓存) */
  input: number
...
export interface ChatgptUsageApi {
  ...
  /** 获取指定日期(本地自然日)的 24 小时 token 用量分布;返回固定 24 元素数组,缺失小时零填充 */
  getTokenUsageHourly: (dateMs: number) => Promise<TokenUsageHour[]>
  /** 获取指定日期某小时的 60 分钟 token 用量分布;返回固定 60 元素数组,缺失分钟零填充 */
  getTokenUsageMinutely: (dateMs: number, hour: number) => Promise<TokenUsageMinute[]>
...
/** 横版双窗口(5h+7d)固定宽度:行宽 122(weekly34+gap4+percent34+gap4+progress46) + 左右 padding 8+8 */
export const DUAL_CAPSULE_WINDOW_WIDTH = 138
