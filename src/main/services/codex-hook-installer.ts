...
const MANAGED_STATUS = 'ChatgptUsage task activity'
// 旧版本 statusMessage 字符串;用于识别并清理历史安装遗留的 hook 组,避免卸载漏网
const LEGACY_MANAGED_STATUSES = ['CodexStatus task activity']
const HOOK_EVENTS = [
  'SessionStart',
  'UserPromptSubmit',
  'PreToolUse',
...
export async function installCodexHooks(options: InstallCodexHooksOptions): Promise<void> {
  const scriptPath = path.join(options.installDirectory, 'chatgpt-usage-hook.cjs')
  const launcherPath = path.join(options.installDirectory, 'chatgpt-usage-hook.cmd')
...
export async function uninstallCodexHooks(
  ...
  await Promise.all([
    fs.unlink(path.join(installDirectory, 'chatgpt-usage-hook.cjs')).catch(() => undefined),
    fs.unlink(path.join(installDirectory, 'chatgpt-usage-hook.cmd')).catch(() => undefined),
    // 清理旧版本残留文件(改名前的产物)
...
function isManagedGroup(group: HookGroup): boolean {
  return group.hooks.some(
    (handler) =>
      handler.statusMessage === MANAGED_STATUS ||
      LEGACY_MANAGED_STATUSES.includes(handler.statusMessage ?? '')
  )
}
