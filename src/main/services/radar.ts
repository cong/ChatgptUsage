...
async function fetchRawEntries(): Promise<{ entries: RadarModelEntry[]; updatedAt?: string } | undefined> {
  try {
    ...
    console.warn('[chatgpt-usage] radar fetch failed:', error instanceof Error ? error.message : error)
    return undefined
  }
}
...
function parseComparisons(response: unknown): { entries: RadarModelEntry[]; updatedAt?: string } | undefined {
  ...
  if (!comparisons || typeof comparisons !== 'object') {
    console.warn('[chatgpt-usage] radar parse: comparisons missing, model_iq keys:', modelIq ? Object.keys(modelIq) : 'model_iq missing')
    return undefined
  }
...
function pickBest(
  ...
  if (threshold < minScore) {
    console.warn(
      `[chatgpt-usage] radar: no model above IQ ${minScore}, auto-lowered to ${threshold} (${qualified.length} candidates)`
    )
  }
