...
/**
 * 指定日期(本地自然日)的 24 小时 token 用量分布。
 * 复用 loadScanned 的带 TTL+指纹缓存(不新扫文件),按 [startMs, endMs) 切片后按本地小时归桶。
 * 返回固定 24 元素数组,索引 0-23 对应小时;缺失小时零填充,保证 UI 拿到稳定长度。
 */
export async function getTokenUsageHourly(
  agentId: AgentId,
  dateMs: number
): Promise<TokenUsageHour[]> {
  const { events } = await loadScanned(agentId)
  // dateMs 所在本地自然日的 [00:00, 次日 00:00)
  const reference = new Date(dateMs)
  ...
  return aggregateHours(events, startMs, endMs)
}

/**
 * 指定日期某小时的 60 分钟 token 用量分布(小时下钻)。
 * 复用 loadScanned 缓存,按 [hourStart, hourStart+1h) 切片后按本地分钟归桶。
 * 返回固定 60 元素数组,索引 0-59 对应分钟;缺失分钟零填充。
 */
export async function getTokenUsageMinutely(
  agentId: AgentId,
  dateMs: number,
  hour: number
): Promise<TokenUsageMinute[]> {
  const { events } = await loadScanned(agentId)
  const reference = new Date(dateMs)
  ...
  return aggregateMinutes(events, hourStart, hourEnd)
}
...
// 事件级明细 → 本地小时桶(24 元素固定长度,索引=小时);成本同 aggregateDays 取 costUsd 或按价目估算
function aggregateHours(events: UsageEvent[], startMs: number, endMs: number): TokenUsageHour[] {
  const buckets: Accum[] = Array.from({ length: 24 }, () => ({
    input: 0,
    cachedInput: 0,
    output: 0,
    reasoning: 0,
...
// 事件级明细 → 本地分钟桶(60 元素固定长度,索引=分钟);成本同 aggregateDays 取 costUsd 或按价目估算
function aggregateMinutes(
  events: UsageEvent[],
  startMs: number,
  endMs: number
): TokenUsageMinute[] {
  const buckets: Accum[] = Array.from({ length: 60 }, () => ({
    input: 0,
    cachedInput: 0,
    output: 0,
    reasoning: 0,
  ...
  for (const event of events) {
    ...
    const minute = new Date(event.ts).getMinutes()
    const acc = buckets[minute]
    acc.input += event.tokens.input
    acc.cachedInput += event.tokens.cachedInput
    acc.output += event.tokens.output
    acc.reasoning += event.tokens.reasoning
