...
// 渲染层最近一次 setCapsuleSize 上报的尺寸;双窗口自适应放大后,拖动/停靠时据此保持该尺寸,避免缩回或沿用另一种模式的旧尺寸
let capsuleRuntimeSize: { width: number; height: number } | null = null
...
function registerIpcHandlers(): void {
  ...
  // 指定日期(本地自然日)的 24 小时 token 用量分布(今日小时分布图)
  ipcMain.handle(CHANNELS.tokenUsageHourly, async (_event, dateMs: number) => {
    return getTokenUsageHourly(persistedState.settings.agentId, dateMs)
  })

  // 指定日期某小时的 60 分钟 token 用量分布(小时下钻分钟分布图)
  ipcMain.handle(CHANNELS.tokenUsageMinutely, async (_event, dateMs: number, hour: number) => {
    return getTokenUsageMinutely(persistedState.settings.agentId, dateMs, hour)
  })
  ...
  ipcMain.handle(
    ...
    async (_event, size: { width: number; height: number }) => {
      ...
      const width = Math.round(clamp(size.width, 40, 480))
      const height = Math.round(clamp(size.height, 28, 480))
      capsuleRuntimeSize = { width, height }
...
function buildTrayTooltip(): string {
  ...
  if (windowTexts.length === 0) {
    return persistedState.settings.locale === 'en-US'
      ? `ChatGPT status unavailable${suffix}`
      : `ChatGPT 暂无额度数据${suffix}`
  }

  return `${['ChatGPT', ...windowTexts].join('  ')}${suffix}`
}
...
async function installIslandHooks(): Promise<void> {
  ...
  await installCodexHooks({
    ...
    sourceScriptPath: app.isPackaged
      ? join(process.resourcesPath, 'hooks', 'chatgpt-usage-hook.cjs')
      : join(app.getAppPath(), 'resources', 'hooks', 'chatgpt-usage-hook.cjs'),
...
function applyCapsuleWindowPreferences(
  ...
  // 模式(横版/竖版)切换时清空渲染层历史上报尺寸,避免吸附/脱离瞬间沿用另一模式的旧宽高
  if (preferences.viewMode !== persistedState.window.viewMode) {
    capsuleRuntimeSize = null
  }
...
function resolveCapsuleWindowSize(viewMode: 'capsule' | 'orb'): {
  ...
  // 胶囊只展示单个窗口(5h 优先,无 5h 取 7d 兜底),默认尺寸不随 rateLimits 数量变化,
  // 避免 5h 回来后窗口从 160 撑到 250 而内容不变产生大片空白。
  // 双窗口模式:渲染层通过 setCapsuleSize 上报自适应尺寸,拖动/停靠时据此保持(避免缩回,
  // 也避免切换模式时沿用另一种模式的旧尺寸)。无上报时回退各模式默认尺寸。
  if (capsuleRuntimeSize) {
    return capsuleRuntimeSize
  }
  if (viewMode === 'orb') {
    return { width: ORB_WINDOW_SIZE.width, height: SINGLE_ORB_WINDOW_HEIGHT }
  }
  // 横版:双窗口(5h+7d)宽度由渲染层固定上报 130,主进程回退也用同值,避免切换瞬间落到单窗口 160 而偏宽
  const isDualWindow = currentSnapshot.rateLimits.length >= 2
  return {
    width: isDualWindow ? DUAL_CAPSULE_WINDOW_WIDTH : SINGLE_CAPSULE_WINDOW_WIDTH,
    height: CAPSULE_WINDOW_SIZE.height
  }
}
