...
export default function Island(): React.JSX.Element {
  ...
  useEffect(() => {
    let active = true
    void window.chatgptUsage.bootstrap().then((payload) => {
      if (!active) return
    ...
    const disposeIsland = window.chatgptUsage.onIslandUpdated((next) => {
      latestSnapshot.current = next
      if (presentationRef.current) setSnapshot(next)
    })
    const disposePresentation = window.chatgptUsage.onIslandPresentation((next) =>
  ...
  useEffect(() => {
    if (rendererReady) void window.chatgptUsage.notifyIslandReady()
  }, [rendererReady])
  ...
  function handlePresentation(next: IslandPresentation): void {
    ...
    exitTimer.current = window.setTimeout(() => {
      setMode('hidden')
      void window.chatgptUsage.notifyIslandHidden(next.revision)
    }, delay)
  }
  ...
  function setIslandInteractive(interactive: boolean): void {
    void window.chatgptUsage.setIslandInteractive(interactive)
  }

  function openTask(task: IslandTask): void {
    reminder.current?.stop()
    setNavigationError('')
    void window.chatgptUsage.openIslandTask(task.threadId).then((confirmed) => {
      if (!confirmed) {
  ...
  function dismissTask(task: IslandTask): void {
    void window.chatgptUsage.dismissIslandTask(task.threadId).then((dismissed) => {
      if (!dismissed) return
