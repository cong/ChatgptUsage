...
const hookPath = path.resolve('resources/hooks/chatgpt-usage-hook.cjs')

test('Hook 客户端通过随机管道上报，审批输出空决定', async (context) => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'chatgpt-usage-hook-'))
  ...
  assert.match(endpoint.pipePath, /chatgpt-usage-hook-/)
