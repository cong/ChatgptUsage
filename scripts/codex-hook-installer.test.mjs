...
test('合并保留用户 Hook 且重复安装不重复', () => {
  const first = mergeCodexHooks(existing, '"C:\\ChatgptUsage\\hook.cmd"')
  const second = mergeCodexHooks(first, '"C:\\ChatgptUsage\\hook.cmd"')
...
test('移除只删除 ChatgptUsage 管理项', () => {
  const merged = mergeCodexHooks(existing, '"C:\\ChatgptUsage\\hook.cmd"')
...
test('安装和关闭只管理自己的文件与配置项', async (context) => {
  ...
  await installCodexHooks({
    ...
    installDirectory,
    sourceScriptPath,
    executablePath: 'C:\\Program Files\\ChatgptUsage.exe',
    descriptorPath: 'C:\\Data\\endpoint.json'
  })
  assert.equal(
    (await fs.readFile(path.join(installDirectory, 'chatgpt-usage-hook.cmd'), 'utf8')).includes(
  ...
  await assert.rejects(fs.access(path.join(installDirectory, 'chatgpt-usage-hook.cjs')))
})
