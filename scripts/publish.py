...
# -*- coding: utf-8 -*-
"""
chatgpt-usage 一键打包并发布到 GitHub Release
...
ARTIFACT_GLOBS = {
    "setup": "ChatgptUsage-{ver}-setup.exe",
    "latest": "latest.yml",
    "blockmap": "ChatgptUsage-{ver}-setup.exe.blockmap",
}
...
def publish(gh: str, version: str, artifacts: dict[str, Path], notes: str | None) -> None:
    ...
    if notes:
        ...
        cmd += ["--notes", f"ChatgptUsage {tag}"]
    run(cmd)
    print(f"\n[已发布] https://github.com/{OWNER_REPO}/releases/tag/{tag}")


def main() -> None:
    parser = argparse.ArgumentParser(description="打包并发布 chatgpt-usage 到 GitHub Release")
