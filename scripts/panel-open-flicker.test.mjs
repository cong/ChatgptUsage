...
test('隐藏 Panel 完成一帧绘制后再发送 ready', () => {
  ...
  assert.match(panelReadyEffect, /void window\.chatgptUsage\.notifyPanelReady\(\)/)
})
...
test('tab 动效只由用户切换到不同页面触发', () => {
  ...
  const commandStart = appSource.indexOf('const disposeCommand = window.chatgptUsage.onCommand')
